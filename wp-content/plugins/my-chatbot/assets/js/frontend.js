...
jQuery(document).ready(function() {
	...
	jQuery('input.myc-text').keyup(function(event) {
        message = jQuery(this).val();

        if (message != "") {
            jQuery('.send-button').addClass('visible');
		} else
            jQuery('.send-button').removeClass('visible');
	});
	...
	jQuery('.chat-toggle').click(function (event) {
		slideToggle();
    });
...
function slideToggle() {
    ...
    if (header.find(".myc-icon-toggle-up").css("display") !== "none") { // toggle open
        ...
        var container = header.siblings(".myc-content-overlay-container").find(".myc-container");
        ...
        header.find(".myc-icon-toggle-up").hide();
        header.parent().removeClass("myc-toggle-closed");
        header.parent().addClass("myc-toggle-open");
        header.find(".myc-icon-toggle-down").show();
        header.siblings(".myc-content-overlay-container, .myc-content-overlay-powered-by").slideToggle("slow", function() {});
    } else { // toggle close
        header.find(".myc-icon-toggle-down").hide();
        header.parent().removeClass("myc-toggle-open");
        header.parent().addClass("myc-toggle-closed");
        header.find(".myc-icon-toggle-up").show();
        header.siblings(".myc-content-overlay-container, .myc-content-overlay-powered-by").slideToggle("slow", function() {});
    }
...
function prepareResponse(response, sequence) {
	...
	if (response.status.code == "200" ) {
		...
		for (index; index<numMessages; index++) {
			...
			if (myc_script_vars.messaging_platform == message.platform || myc_script_vars.messaging_platform == "default" && message.platform === undefined || message.platform === undefined && ! hasPlatform(messages, myc_script_vars.messaging_platform) ) {
